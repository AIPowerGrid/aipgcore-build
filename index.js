 ...
 * <ul>
 * <li> `browser:uncompressed` - build uncomprssed browser bundle (`aipgcore-*.js`)
 * <li> `browser:compressed` - build compressed browser bundle (`aipgcore-*.min.js`)
...
function startGulp(name, opts) {
  ...
  var fullname = name ? 'aipgcore-' + name : 'aipgcore';
  var files = ['lib/**/*.js'];
  var tests = ['test/**/*.js'];
  var alljs = files.concat(tests);

  var buildPath = './node_modules/aipgcore-build/';
  ...
  if (browser) {
    ...
    if (name !== 'lib') {
      browserifyCommand = browserifyPath + ' --require ./index.js:' + fullname + ' --external aipgcore-lib -o ' + fullname + '.js';
    } else {
      browserifyCommand = browserifyPath + ' --require ./index.js:aipgcore-lib -o aipgcore-lib.js';
    }
